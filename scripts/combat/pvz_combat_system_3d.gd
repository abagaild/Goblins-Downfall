extends Node3D
class_name PVZCombatSystem3D

# Grid configuration
@export var grid_width: int = 9
@export var grid_height: int = 5
@export var cell_size: Vector3 = Vector3(2.0, 0.0, 2.0)  # X,Z plane for ground

# Combat stats
@export var base_sun_generation: float = 25.0
@export var sun_generation_interval: float = 10.0
...
func initialize_grid():
    ...
    for z in range(grid_height):
        ...
        for x in range(grid_width):
            var cell = {
                "position": Vector2(x, z),  # Store as 2D for compatibility
                "occupied": false,
                "plant": null,
                "enemies": []
            }
...
func generate_sun():
    ...
    # Optionally spawn sun objects that fall from the sky
    if grid_container:
        var sun_scene = load("res://scenes/combat/sun_3d.tscn")
        if sun_scene:
            var sun = sun_scene.instantiate()
            grid_container.add_child(sun)
            
            # Random position above the grid
            var x = randf_range(0, grid_width * cell_size.x)
            sun.position = Vector3(x, 10.0, 0)  # Start high above
            sun.value = base_sun_generation
...
func place_plant(grid_x, grid_y):
    ...
    var plant = load("res://scenes/combat/plants_3d/" + selected_plant + ".tscn").instantiate()
    grid_container.add_child(plant)
    
    # Position plant in 3D space
    plant.position = Vector3(grid_x * cell_size.x, cell_size.y, grid_y * cell_size.z)
...
func spawn_enemy(enemy_type, lane):
    ...
    var enemy_path = "res://scenes/combat/enemies_3d/" + enemy_type + ".tscn"
    ...
    enemy.position = Vector3(grid_width * cell_size.x, cell_size.y, lane * cell_size.z)
...
func _on_plant_destroyed(plant):
    ...
    for z in range(grid_height):
        for x in range(grid_width):
            if grid_cells[z][x].plant == plant:
                grid_cells[z][x].occupied = false
                grid_cells[z][x].plant = null
                break
...
func _on_enemy_reached_end(enemy):
    ...
    # Game over - enemies reached the end
